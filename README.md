...
AlphaDelta는 미국 상장기업의 SEC 공시를 기반으로, 단순한 공시 요약을 넘어 이전 공시와 무엇이 달라졌는지(Delta)를 분석하는 것을 목표로 시작한 프로젝트입니다.
...
따라서 AlphaDelta는 이 문제를 다음과 같이 해결하는 것을 목표로 합니다.
...
AlphaDelta의 이름은 다음과 같은 개념에서 출발했습니다.
...
각 사례에 대해 현재 사건과의 차이점, 당시 대응 방식, 비즈니스에 미친 영향을
사실 기반으로 제공합니다.
